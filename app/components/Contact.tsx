    ...
    return (
        <section id="contact" className="py-[90px] bg-white">
            <div className="max-w-[1100px] mx-auto px-6 sm:px-10 lg:px-12 grid lg:grid-cols-[.95fr_1.05fr] gap-12 items-start">
                ...
                <form
                    onSubmit={(e) => { e.preventDefault(); showToast("Thank you — inquiry sent to Chinny's studio inbox."); (e.target as HTMLFormElement).reset(); }}
                ...
                >
                    <div className="grid sm:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input required className="mt-1 w-full px-[13px] py-[11px] rounded-xl border border-[#e4c6d7] bg-white text-[14px] outline-none focus:border-[#B6178C]" />
                        </div>
                        <div>
                            <label className="text-[12.5px] font-[600] text-[#623b57]">Email</label>
                            <input type="email" required className="mt-1 w-full px-[13px] py-[11px] rounded-xl border border-[#e4c6d7] bg-white text-[14px] outline-none focus:border-[#B6178C]" />
                        </div>
                        <div>
                            <label className="text-[12.5px] font-[600] text-[#623b57]">Service interest</label>
                            <select className="mt-1 w-full px-[13px] py-[11px] rounded-xl border border-[#e4c6d7] bg-white text-[14px] outline-none focus:border-[#B6178C]">
                        ...
                        <div>
                            ...
                            <input placeholder="e.g. Spring 2026" className="mt-1 w-full px-[13px] py-[11px] rounded-xl border border-[#e4c6d7] bg-white text-[14px] outline-none focus:border-[#B6178C]" />
                        </div>
                        <div className="sm:col-span-2">
                            <label className="text-[12.5px] font-[600] text-[#623b57]">Project / message</label>
                            <textarea required rows={4} placeholder="Tell Chinny about your wall, space, vision, budget range…" className="mt-1 w-full px-[13px] py-[11px] rounded-xl border border-[#e4c6d7] bg-white text-[14px] outline-none focus:border-[#B6178C] resize-y" />
                        </div>
                    </div>
                    <button className="mt-5 w-full py-[13px] rounded-full text-white font-[660] text-[15px]"
                        style={{ background: 'linear-gradient(135deg,#5C1466,#B6178C 60%, #E454B0)', boxShadow: '0 10px 26px rgba(182,23,140,0.27)' }}>
                        Send inquiry to @rt Chinny World
                    </button>
                    <div className="text-[11.7px] text-[#9b748f] text-center mt-3">No checkout. Price-on-request. You'll receive a personal studio reply.</div>
                </form>
            </div>
        </section>
    );
